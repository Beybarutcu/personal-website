        ...
        <div className="h-80 bg-gradient-to-br from-gray-700 to-gray-900 relative overflow-hidden">
          ...
          {(!project.image || imageError) && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
              <div className="mb-6 p-6 rounded-full bg-white/10 backdrop-blur-sm">
                {/* Tech-specific icons - larger for modal */}
                {project.technologies[0]?.toLowerCase().includes('flutter') && (
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {project.technologies.slice(0, 3).map((tech, index) => (
                  <span 
                    key={index}
                    className="px-3 py-1 text-sm bg-white/20 rounded text-gray-300"
                  >
                    {tech}
                  </span>
                ))}
          ...
          {/* Dark overlay for better text readability (when image is loaded) */}
          {imageLoaded && (
